...
<body class="h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8">
                ...
                <div>
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        ...
                        <button type="button" onclick="togglePasswordVisibility()"
                            class="absolute inset-y-0 right-0 pr-3 flex items-center hover:text-slate-800 text-slate-400 cursor-pointer">
    ...
    async function handleLoginSubmit(event) {
        ...
        btnSubmit.innerHTML =
            '<span>ĐANG CHỨNG THỰC...</span> <div class="animate-spin rounded-full h-4 w-4 border-2 border-white/30 border-t-white"></div>';
        hideAlert();
        ...
        try {
            ...
            if (response.ok && data.status === 'success') {
                ...
                showAlert('danger', data.messages?.error || data.message ||
                    'Mật danh hoặc Mật khẩu không trùng khớp.');
                btnSubmit.disabled = false;
                btnSubmit.innerHTML =
                    '<span>ĐĂNG NHẬP HỆ THỐNG</span> <i data-lucide="arrow-right-circle" class="w-4 h-4"></i>';
                lucide.createIcons();
            }
        } catch (err) {
            console.error(err);
    ...
    function showAlert(type, message) {
        ...
        if (type === 'success') {
            alertBlock.className =
                'mb-5 p-3 rounded-xl border flex items-start gap-2.5 text-xs bg-emerald-50 border-emerald-200 text-emerald-800';
            alertIcon.setAttribute('data-lucide', 'check-circle-2');
        } else {
            alertBlock.className =
                'mb-5 p-3 rounded-xl border flex items-start gap-2.5 text-xs bg-rose-50 border-rose-200 text-rose-800';
